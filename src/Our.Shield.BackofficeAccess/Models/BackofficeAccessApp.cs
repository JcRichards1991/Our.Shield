...
namespace Our.Shield.BackofficeAccess.Models
{
	...
	public class BackofficeAccessApp : App<BackofficeAccessConfiguration>
	{
		...
		private void AddSoftWatches(IJob job, BackofficeAccessConfiguration config)
		{
			...
			var umbracoRegex = job.PathToRegex(umbracoLocation);
			job.IgnoreWebRequest(umbracoRegex);
			...
			//add watch on the soft location
			var softRegex = job.PathToRegex(softLocation);
			job.IgnoreWebRequest(softRegex);
			SoftWatcher(job,
				softRegex,
				20010,
				hardLocation,
			...
			var hardRegex = job.PathToRegex(hardLocation);
			job.IgnoreWebRequest(hardRegex);

			//Add watch on the hard location
			job.WatchWebRequests(PipeLineStages.AuthenticateRequest, hardRegex, 20020, (count, httpApp) =>
			{
		...
		{
			var hardRegex = job.PathToRegex(Configuration.UmbracoPath.EnsureStartsWith('/').EnsureEndsWith('/'));
			job.IgnoreWebRequest(hardRegex);
			...
			job.WatchWebRequests(PipeLineStages.AuthenticateRequest, hardRegex, 21000, (count, httpApp) =>
			{
				if (AccessHelper.IsRequestAuthenticatedUmbracoUser(httpApp))
				{
					return new WatchResponse(WatchResponse.Cycles.Continue);
