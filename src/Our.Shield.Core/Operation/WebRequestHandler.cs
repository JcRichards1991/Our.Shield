...
namespace Our.Shield.Core.Operation
{
	...
	internal class WebRequestHandler : IHttpModule
	{
		...
		private bool IgnoresProcess()
		{
			...
			{
				foreach (var exception in UrlIgnores.Where(x => x.CalculatedUrl == false && x.Url != null))
				{
					UrlProcess(exception);
				}
			});
		...
		private WatchResponse.Cycles ExecuteTransfer(int environmentId, Watcher watch, WatchResponse response, HttpApplication application)
		{
			...
			//if (response.Transfer.TransferType == TransferTypes.Rewrite && response.Transfer.Url.Type != UmbracoUrlTypes.Url)
			//{
			//	application.Context.RewritePath("/Umbraco_Client/Ui/jquery.js");
			//}
			...
			switch (response.Transfer.TransferType)
			{
				...
				case TransferTypes.Rewrite:
					...
					{
						var MyClient = WebRequest.Create(application.Request.Url.GetLeftPart(UriPartial.Authority) + url) as HttpWebRequest;
						MyClient.Method = application.Request.HttpMethod;
						//foreach (var key in application.Request.Headers.AllKeys)
						//{
						//	MyClient.Headers.Set(key, application.Request.Headers.Get(key));
						//}
						var result = MyClient.GetResponse() as HttpWebResponse;
